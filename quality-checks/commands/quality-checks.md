...
Load `quality-checks/PIPELINE.md`, `RUBRIC.md`, `CHECKLIST.md`, `ANTIPATTERNS.md`, `COPY-DENYLIST.md`. `BLOCKS.md` only for paste-replacement.

## Discovery (Phase 0)

Register. Design system map. Stack. Industry (use this repo's `search.py` when present: `python3 src/ui-ux-pro-max/scripts/search.py "<title>" --domain product`). Anti-references.
...
Sub-tasks for Phase 1 (security; invoke `/security-review` if available; confidence ≥ 8), Phase 4 (a11y per file), Phase 8 (stack per file).

## Composite & sign-off

Score /20 from Anti-Pattern + Design System + Accessibility + Performance + Resilience. Severity census. Security verdict. Gate verdict. Final per `RUBRIC.md`. Recommended commands.
